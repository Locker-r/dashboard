...
| B4 | P1 (external, done) | Product Owner decision (2026-08-07): GitHub Pages is approved for staging/pilot only, at the canonical URL `https://locker-r.github.io/dashboard/`, published by `.github/workflows/staging-pages-deploy.yml` (manual dispatch). Product Owner decision (2026-08-08, ADR-013): Cloudflare Pages is the approved **production** host. Operator evidence (2026-08-08, ADR-014): the Cloudflare account and Pages project (`dashboard-latam`, `dashboard-latam.pages.dev`) are provisioned, and the approved deployment mechanism is Direct Upload via a reviewed GitHub Actions workflow using Wrangler, authenticated by secrets in the GitHub `production` environment. | `release/backlog.json` B4; `docs/release-gates.md` "Frontend hosting: GitHub Pages, staging/pilot only" and "Frontend hosting: production (Cloudflare Pages)"; `docs/decisions.md` ADR-013, ADR-014. | Remaining, deferred to M-D2B: author and run the actual publishing workflow. |
