...
| B4 | P1 (external) | Product Owner decision (2026-08-07): GitHub Pages is approved for staging/pilot only, at the canonical URL `https://locker-r.github.io/dashboard/`. Production hosting remains undecided. No Pages deployment workflow exists yet — `release.yml` still deploys nothing — so the artifact is not yet published there. | `release/backlog.json` B4; `docs/release-gates.md` "Frontend hosting: GitHub Pages, staging/pilot only". | Remaining: add the reviewed publishing workflow and dispatch it; separately decide production hosting. |
