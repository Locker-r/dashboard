...
| B4 | P1 (external) | Product Owner decision (2026-08-07): GitHub Pages is approved for staging/pilot only, at the canonical URL `https://locker-r.github.io/dashboard/`, published by `.github/workflows/staging-pages-deploy.yml` (manual dispatch). Product Owner decision (2026-08-08, ADR-013): Cloudflare Pages is the approved **production** host — the choice only; account creation, project creation, and a deployment mechanism remain undone. | `release/backlog.json` B4; `docs/release-gates.md` "Frontend hosting: GitHub Pages, staging/pilot only" and "Frontend hosting: production (Cloudflare Pages), account and deployment mechanism not yet configured"; `docs/decisions.md` ADR-013. | Remaining, external: create the Cloudflare account/project and choose a deployment mechanism. |
