-- Rollback for phase 1 (20260801000100_secure_contact_projection.sql).
-- Apply the phase 2 rollback first if that migration was applied, otherwise authenticated is left with the
-- restricted column grant and no projection to read.
-- Restores the four mutation RPCs to their pre-PR-A shape returning public.players, then drops the
-- projection, the duplicate-check contract and the helper functions.
begin;
...
drop function if exists public.check_player_duplicates(jsonb);
drop function if exists public.can_read_player(uuid);
drop function if exists public.mask_contact_phone(text);
drop function if exists public.mask_contact_email(text);
drop function if exists public.mask_contact_messenger(text);
drop function if exists public.normalize_contact_phone(text);
drop function if exists public.normalize_contact_email(text);
drop function if exists public.normalize_contact_messenger(text);
