...
function buildPlan(runId) {
  ...
  return Object.freeze({
    runId, prefix, marker: `${PREFIX}:${runId}`,
...
async function runSmoke(config) {
  ...
  try {
    ...
    await adminData.createPlayers([
      { id: plan.assignedPlayerId, email: `smoke_test_${plan.runId}_assigned@example.invalid`, messenger: plan.marker },
      { id: plan.otherPlayerId, email: `smoke_test_${plan.runId}_other@example.invalid`, messenger: plan.marker }
    ]);
    writesStarted = true;
    ...
    await expectRpcFailure(admin.client, 'create_players_atomic', { p_players: [{
      id: plan.assignedPlayerId, email: `smoke_test_${plan.runId}_duplicate@example.invalid`, messenger: plan.marker
    }] }, 'PLAYER_ID_CONFLICT');
    return { ok: true, runId: plan.runId };
  } finally {
    if (admin && writesStarted) {
      await cleanup(admin.client, plan);
...
module.exports = { PREFIX, WRITE_CONFIRMATION, STAGING_CONFIRMATION, RUN_ID_PATTERN, normalizeUrl, isLoopback, validateRunId, readConfig, buildPlan };
