...
function Invoke-CheckedCommand {
  param([string]$Name,[string]$FilePath,[string[]]$Arguments=@(),[bool]$Required=$true,[int]$TimeoutSeconds=120,[string]$WorkingDirectory=(Get-Location).Path,[ValidateSet('Terminate','Wait')][string]$TimeoutPolicy='Terminate')
  ...
  try {
    ...
    if(-not$process.WaitForExit($TimeoutSeconds*1000)){if($TimeoutPolicy-eq'Wait'){$process.WaitForExit();$details=(Protect-SensitiveText (($stdoutTask.Result,$stderrTask.Result|Where-Object{$_})-join"`n")).Trim();return New-CheckResult $Name 'Failed' $Required $watch.Elapsed.TotalSeconds "Exceeded $TimeoutSeconds seconds; the destructive process tree was allowed to finish safely. Exit $($process.ExitCode). $details" $display};try{$process.Kill()}catch{};return New-CheckResult $Name 'Failed' $Required $watch.Elapsed.TotalSeconds "Timed out after $TimeoutSeconds seconds; immediate child termination was requested." $display}
